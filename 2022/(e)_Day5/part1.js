...
async function task(input) {
	...
	for (let i = 0; i < allInstructions.length; i++) {
		...
		// remove N stacks from the source stack (one by one!) and add them to destination stack
		for (let j = 0; j < normalInstruction.howManyStacksToMove; ++j) {
			stacks[normalInstruction.destinationStack].unshift(
				stacks[normalInstruction.sourceStack].shift()
			);
		}
